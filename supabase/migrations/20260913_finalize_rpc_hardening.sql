...
-- The API server already verifies the Supabase JWT. Keep the mutation RPC
-- server-only so clients cannot bypass API policy or tune cooldown values.
...
-- Remove direct client execution from all legacy check-in RPCs. The currently
-- deployed server still uses increment_place_checkins through service_role, so
-- keep that service-role grant until the new server deploy completes.
...
-- Legacy authenticated AI quota RPCs are superseded by the server-only hashed
-- api_rate_limits implementation. Prevent direct browser calls.
...
-- Keep an explicit deny policy so the intent is visible to the linter while the
-- table remains accessible only to service_role (which bypasses RLS).
