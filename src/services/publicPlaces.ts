...
type RankedPlace = Place & { _score?: number };
...
function osmElementToPlace(element: any, area: { city: string; region: string }, origin: [number, number]): RankedPlace | null {
...
async function fetchOverpassPlaces(latitude: number, longitude: number, area: { city: string; region: string }, fetcher: FetchLike) {
...
function dedupeAndRank(places: RankedPlace[]) {
...
export async function discoverNearbyPublicPlaces(options: {
  ...
  if (places.length < 5) places = places.concat(await fetchWikipediaPlaces(latitude, longitude, language, area, fetcher));
  let result = dedupeAndRank(places);
  if (category) result = result.filter((place) => place.category === category);
  cache.set(cacheKey, { expiresAt: Date.now() + CACHE_TTL_MS, places: result });
